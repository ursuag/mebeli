object F_Cassa_rashod: TF_Cassa_rashod
  Left = 130
  Top = 70
  AutoScroll = False
  Caption = #1056#1072#1089#1093#1086#1076' '#1087#1086' '#1082#1072#1089#1089#1077
  ClientHeight = 702
  ClientWidth = 1184
  Color = clBtnFace
  Font.Charset = RUSSIAN_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Arial'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  WindowState = wsMaximized
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 18
  object Label1: TLabel
    Left = 8
    Top = 56
    Width = 17
    Height = 18
    Caption = #8470
  end
  object Label2: TLabel
    Left = 208
    Top = 8
    Width = 154
    Height = 19
    Caption = #1056#1040#1057#1061#1054#1044' '#1055#1054' '#1050#1040#1057#1057#1045
    Font.Charset = RUSSIAN_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel
    Left = 128
    Top = 56
    Width = 34
    Height = 18
    Caption = #1076#1072#1090#1072
  end
  object Label4: TLabel
    Left = 8
    Top = 152
    Width = 50
    Height = 18
    Caption = #1057#1091#1084#1084#1072
  end
  object Label5: TLabel
    Left = 205
    Top = 152
    Width = 25
    Height = 18
    Caption = #1083#1077#1081
  end
  object Label7: TLabel
    Left = 5
    Top = 193
    Width = 81
    Height = 18
    Caption = #1050#1086#1085#1090#1088#1072#1075#1077#1085#1090
  end
  object B_Contragent_clear: TSpeedButton
    Left = 488
    Top = 188
    Width = 27
    Height = 24
    Hint = #1054#1095#1080#1089#1090#1080#1090#1100' '#1087#1086#1083#1077' '#1050#1086#1085#1090#1088#1072#1075#1077#1085#1090
    Glyph.Data = {
      
      3333333333333333333333333333333333333333333333333333}
    NumGlyphs = 2
  end
  object Label6: TLabel
    Left = 5
    Top = 233
    Width = 75
    Height = 18
    Caption = #1057#1086#1090#1088#1091#1076#1085#1080#1082
  end
  object B_Sotrudnik_Clear: TSpeedButton
    Left = 488
    Top = 228
    Width = 27
    Height = 24
    Hint = #1054#1095#1080#1089#1090#1080#1090#1100' '#1087#1086#1083#1077' '#1057#1086#1090#1088#1091#1076#1085#1080#1082
    Glyph.Data = {
      
      3333333333333333333333333333333333333333333333333333}
    NumGlyphs = 2
  end
  object Label8: TLabel
    Left = 5
    Top = 265
    Width = 117
    Height = 18
    Caption = #1057#1090#1072#1090#1100#1103' '#1088#1072#1089#1093#1086#1076#1072
  end
  object Label9: TLabel
    Left = 8
    Top = 96
    Width = 89
    Height = 18
    Caption = #1055#1088#1080#1084#1077#1095#1072#1085#1080#1077
  end
  object P_Buttons: TPanel
    Left = 0
    Top = 640
    Width = 1177
    Height = 57
    BevelInner = bvRaised
    BevelOuter = bvLowered
    TabOrder = 0
    object B_Exit: TButton
      Left = 1058
      Top = 12
      Width = 97
      Height = 33
      Caption = #1047#1072#1082#1088#1099#1090#1100
      TabOrder = 1
      OnClick = B_ExitClick
    end
    object B_Ok: TButton
      Left = 954
      Top = 12
      Width = 97
      Height = 33
      Hint = '[Ctrl+End] '#1089#1086#1093#1088#1072#1085#1080#1090#1100' '#1080' '#1074#1099#1081#1090#1080
      Caption = #1057#1086#1093#1088#1072#1085#1080#1090#1100
      Default = True
      TabOrder = 0
      OnClick = B_OkClick
    end
  end
  object DBEdit1: TDBEdit
    Left = 32
    Top = 48
    Width = 89
    Height = 26
    TabStop = False
    DataField = 'ID'
    DataSource = DS_Bank_Prihod
    ReadOnly = True
    TabOrder = 1
  end
  object DBEdit3: TDBEdit
    Left = 64
    Top = 144
    Width = 137
    Height = 26
    DataField = 'SUMMA'
    DataSource = DS_Bank_Prihod
    TabOrder = 2
  end
  object DBLookupComboBox3: TDBLookupComboBox
    Left = 98
    Top = 189
    Width = 384
    Height = 26
    DataField = 'CONTRAGENT_NAME'
    DataSource = DS_Bank_Prihod
    TabOrder = 3
  end
  object DBLookupComboBox1: TDBLookupComboBox
    Left = 98
    Top = 229
    Width = 384
    Height = 26
    DataField = 'SOTRUDNIK_NAME'
    DataSource = DS_Bank_Prihod
    TabOrder = 4
  end
  object DBLookupComboBox2: TDBLookupComboBox
    Left = 122
    Top = 261
    Width = 360
    Height = 26
    DataField = 'STATYA_NAME'
    DataSource = DS_Bank_Prihod
    TabOrder = 5
  end
  object DBEdit4: TDBEdit
    Left = 104
    Top = 88
    Width = 553
    Height = 26
    DataField = 'DESCRIPTION'
    DataSource = DS_Bank_Prihod
    TabOrder = 6
  end
  object DateTimePicker1: TDateTimePicker
    Left = 176
    Top = 48
    Width = 137
    Height = 26
    Date = 42725.570340208330000000
    Time = 42725.570340208330000000
    TabOrder = 7
    OnExit = DateTimePicker1Exit
  end
  object DS_Bank_Prihod: TDataSource
    DataSet = IB_Cassa_Rashod
    Left = 632
    Top = 216
  end
  object IB_Cassa_Rashod: TIBDataSet
    Database = DM_Mebeli.DB_Mebeli
    Transaction = DM_Mebeli.IBTransaction1
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from cassa_rashod'
      'where'
      '  ID = :OLD_ID')
    InsertSQL.Strings = (
      'insert into cassa_rashod'
      
        '  (ID, DATE_CAS, SUMMA, ID_CONTRAGENT, ID_SOTRUDNIK, ID_STATYA_R' +
        'ASHODA, '
      '   DESCRIPTION)'
      'values'
      
        '  (:ID, :DATE_CAS, :SUMMA, :ID_CONTRAGENT, :ID_SOTRUDNIK, :ID_ST' +
        'ATYA_RASHODA, '
      '   :DESCRIPTION)')
    RefreshSQL.Strings = (
      'Select *'
      'from cassa_rashod '
      'where'
      '  ID = :ID')
    SelectSQL.Strings = (
      'select * from cassa_rashod'
      'where id=:id')
    ModifySQL.Strings = (
      'update cassa_rashod'
      'set'
      '  ID = :ID,'
      '  DATE_CAS = :DATE_CAS,'
      '  SUMMA = :SUMMA,'
      '  ID_CONTRAGENT = :ID_CONTRAGENT,'
      '  ID_SOTRUDNIK = :ID_SOTRUDNIK,'
      '  ID_STATYA_RASHODA = :ID_STATYA_RASHODA,'
      '  DESCRIPTION = :DESCRIPTION'
      'where'
      '  ID = :OLD_ID')
    Left = 632
    Top = 168
    object IB_Cassa_RashodID: TIntegerField
      FieldName = 'ID'
      Origin = 'BANK_PRIHOD.ID'
      Required = True
    end
    object IB_Cassa_RashodSUMMA: TIBBCDField
      FieldName = 'SUMMA'
      Origin = 'BANK_PRIHOD.SUMMA'
      Required = True
      Precision = 18
      Size = 2
    end
    object IB_Cassa_RashodID_CONTRAGENT: TIntegerField
      FieldName = 'ID_CONTRAGENT'
      Origin = 'BANK_PRIHOD.ID_CONTRAGENT'
    end
    object IB_Cassa_RashodID_SOTRUDNIK: TIntegerField
      FieldName = 'ID_SOTRUDNIK'
      Origin = 'BANK_PRIHOD.ID_SOTRUDNIK'
    end
    object IB_Cassa_RashodCONTRAGENT_NAME: TStringField
      FieldKind = fkLookup
      FieldName = 'CONTRAGENT_NAME'
      LookupDataSet = DM_Mebeli.IB_Contragenty_1
      LookupKeyFields = 'ID'
      LookupResultField = 'NAME'
      KeyFields = 'ID_CONTRAGENT'
      Size = 200
      Lookup = True
    end
    object IB_Cassa_RashodSOTRUDNIK_NAME: TStringField
      FieldKind = fkLookup
      FieldName = 'SOTRUDNIK_NAME'
      LookupDataSet = DM_Mebeli.IB_Sotrudniki
      LookupKeyFields = 'ID'
      LookupResultField = 'FIO'
      KeyFields = 'ID_SOTRUDNIK'
      Size = 200
      Lookup = True
    end
    object IB_Cassa_RashodSTATYA_NAME: TStringField
      FieldKind = fkLookup
      FieldName = 'STATYA_NAME'
      LookupDataSet = DM_Mebeli.IB_Statyi_Rashoda
      LookupKeyFields = 'ID'
      LookupResultField = 'NAME'
      KeyFields = 'ID_STATYA_RASHODA'
      Size = 200
      Lookup = True
    end
    object IB_Cassa_RashodDESCRIPTION: TIBStringField
      FieldName = 'DESCRIPTION'
      Origin = 'BANK_PRIHOD.DESCRIPTION'
      Size = 200
    end
    object IB_Cassa_RashodDATE_CAS: TDateField
      FieldName = 'DATE_CAS'
      Origin = 'CASSA_PRIHOD.DATE_CAS'
      Required = True
    end
    object IB_Cassa_RashodID_STATYA_RASHODA: TIntegerField
      FieldName = 'ID_STATYA_RASHODA'
      Origin = 'CASSA_RASHOD.ID_STATYA_RASHODA'
      Required = True
    end
  end
end
